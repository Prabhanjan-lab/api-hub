...
import testCases.oAuthTokenGeneration.KeystoreAndSSLConnection;
...
public class UploadInstallbase_OCU3_VW_CarportVin{
	
	public static ValidatableResponse putRequestForUploadIB() throws IOException {
		
		String filePath = "resources/Payload_Xmls/OCU3_VW_Carport/OCU3_VW_Carport_Installbase.xml"; // Read the XML content from the file
		...
		request.header("Authorization","Bearer " + TokenGenerationForCarportVins.accessToken)
			   .header("Accept", "application/xml", "application/vnd.vwg.mbb.serviceList_v4_1_2+xml")
			   .header("Content-Type", "application/xml", "application/vnd.vwg.mbb.installedBase_v3_0_3+xml");
		ValidatableResponse response =  request.body(InstallBase_OCU3VWWithChecksum).when().put("/mbb/services/v1/uploadIB/"+TestVariables.checksum).then().log().all();
		return response;	
	}
	
	public static void assertInstallbase_OCU3VW_Carport() throws IOException {	
		ValidatableResponse ib= UploadInstallbase_OCU3_VW_CarportVin.putRequestForUploadIB();
