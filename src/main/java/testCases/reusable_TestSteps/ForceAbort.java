...
public class ForceAbort extends JWT{

	public static ValidatableResponse ForceAbortCampaign(String testname,String vin,String campaign,String criterion) throws IOException {
		...
		ValidatableResponse response = request.when().post("/onup/api/bs/oru/v1/"+brand+"/vehicle/"+vin+"/campaign/"+campaign+"/criterion/"+criterion+"/forceAbort").then().log().all();
		return response;
		}
	...
	public static void assertForceAbort(String testname,String vin,String expectedCampaign,String criterion) throws IOException {
		ValidatableResponse abort= ForceAbort.ForceAbortCampaign(testname,vin,expectedCampaign,criterion);
		abort.assertThat().statusCode(200);
		Response status= ForceAbort.CampaignStatus(testname,vin);
