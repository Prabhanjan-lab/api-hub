...
public class PutVC_RepairNecessary_TM1VW extends RegisterAndGetOAUTH{
	
	public static ValidatableResponse putResquestForPutVc(String testname, String vin) throws IOException{
		
		String filePath = "resources/Payload_Xmls/TM1_VW/TM1VW_PUTVC_RepairNecessary.xml"; // Read the XML content from the file
		String PutVC_TM1RepairNecessary= new String(Files.readAllBytes(Paths.get(filePath)));
		...
		request.header("Authorization","Bearer " + OAuthToken)
			...
			.header("X-Vin",vin);
		ValidatableResponse response = request.body(PutVC_TM1RepairNecessary).when().put("/mbb/vehicleConfiguration/v1/vehicleConf/"+PutVCHash).then().log().all();
		return response;
	}
	
	public static void assertPutVC_TM1VW(String testname, String vin) throws IOException {
		ValidatableResponse putvc= PutVC_RepairNecessary_TM1VW.putResquestForPutVc(testname, vin);
