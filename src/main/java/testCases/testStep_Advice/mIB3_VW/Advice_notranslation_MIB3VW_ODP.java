...
public class Advice_notranslation_MIB3VW_ODP extends RegisterAndGetOAUTH {
	...
	public static ValidatableResponse postRequestForAdvice(String testname, String vin, String campaign, String criterion) throws IOException {
		...
		request.header("Authorization", "Bearer " + OAuthToken)
				...
				.header("Content-Type", "application/json").header("X-Brand", "VW").header("X-Vin", vin)
				.queryParam("lang", "fr_FR").queryParam("vc", PutVCHash)
	...
	public static void assertAdviceRepairNecessary_MIB3VW_ODP(String testname, String vin, String campaign, String criterion) throws IOException {
		ValidatableResponse advice = Advice_notranslation_MIB3VW_ODP.postRequestForAdvice(testname, vin, campaign, criterion);
		advice.assertThat().statusCode(200);
		advice.and().body("repairAdvice.adviceCode", Matchers.equalTo("ABORT_REPAIR"));
		advice.and().assertThat().header("X-Warning",
				"No translated campaign description found for HMI language fr_FR");
	}
