...
public class OUQA_62214_RECALL2_REG_UNECE_VW_OCU3_Vehicle_Overview_After_Recall_import_Dataset_Type_02_for_a_ReCall_Criterion_from_No_to_Yes_VCSO_remains_not_equal_to_55 extends AbstractStandardBehaviour {
	String logPath = "";
	...
	protected void setUp() throws Throwable {
		...
		try {
			...
			this.initTestContainer(4); // test has four test steps
		}
	}
	...
	protected void testHook() throws Throwable {
		...
		this.setLogPath(logPath);
		CreateImportedRecall.createImportedRecall2(campaign1,"recall-ouqa-62214-b-minimal-data-record.json");
		CreateImportedRecall.createImportedRecall2(campaign2,"recall-ouqa-62214-b-minimal-data-record.json");
		...
		logPath = LogReports.logGeneration(testname, "SetTrueDST02-04");
		this.setLogPath(logPath);
		CreateImportedRecall.createImportedRecall2(campaign1, "recall-ouqa-62214-c-minimal-data-record.json");
		CreateImportedRecall.createImportedRecall2(campaign2, "recall-ouqa-62214-c-minimal-data-record.json");
		this.logStepPassed(logPath);
		
		logPath = LogReports.logGeneration(testname, "CheckVCSO2-5");
		this.setLogPath(logPath);
		ActiveQueueVCSOStatus.assertActiveQueueWaitTime(testname, vin, campaign1, "5");
		ScheduledQueue.assertScheduledCampaign(testname, vin, campaign2);
		this.logStepPassed(logPath);
		
		logPath = LogReports.logGeneration(testname, "BatchStatus");
		this.setLogPath(logPath);
		Thread.sleep(60000);
		DefaultBatchStatus.assertDefaultBatchStatusReleased(testname, campaign1, criterion);
		DefaultBatchStatus.assertDefaultBatchStatusReleased(testname, campaign2, criterion);
		this.logStepPassed(logPath);
